...
public class Particle {
  ...
  public void moveVerticalCorkscrew(
      Point4D center, double circleFrequency, double height, double duration) {
    final FiniteTrajectory4d horizontalCircle =
        TrajectoryComposite.builder()
            .addTrajectory(createHorizontalCircleTrajectory(current, center, circleFrequency))
            .withDuration(duration)
            .build();
    this.addMovement(
        VerticalMovementDecorator.create(horizontalCircle, (height - current.getZ()) / duration));
  }

  private static Trajectory4d createHorizontalCircleTrajectory(
      Point4D currentPoint, Point4D center, double frequency) {
  ...
  public void moveTowardPointAndStopRandomlyBeforeReachingPoint(
    ...
    final double duration =
        durationLowerBound
            + (durationUpperBound - durationLowerBound) * RANDOM_GENERATOR.nextDouble();
  ...
  public void moveTowardPointAndStopRandomlyWithInRange(
    ...
    final Point4D stoppingPosition =
        Point4D.create(
            destination.getX() + rangeX,
            destination.getY() + rangeY,
            destination.getZ() + rangeZ,
