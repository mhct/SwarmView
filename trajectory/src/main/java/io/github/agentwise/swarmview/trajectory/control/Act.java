...
/**
 * This class represents an act of a show. An act has the choreographies of several drones,
 * specifying the initial and final position of each drone, involved in the dance show.
 *
 * <p>If a drone is not involved in a particular act, its location still has to be added to the act,
 * to avoid possible collisions.
 *
 * <p>An act always assumes its time to start at ZERO (0.0) TODO add builder to finalize the
 * creation of an Act and calculate its duration
 *
 * @author Mario h.c.t.
 */
public class Act {
  ...
  /**
   * Adds a trajectory of a particular drone to this act. Each drone participating in this act MUST
   * add its trajectory in the act, using this method. Only ONE trajectory per drone.
   *
   * @param droneName
   * @param trajectory
   */
      ...
      if (e.getValue().getTrajectoryDuration() - getDuration() <= -0.001) {
        ...
        e.setValue(
            TrajectoryComposite.builder()
                ...
                .addTrajectory(HoldPositionTrajectory4D.create(finalPosition(e.getKey())))
                .withDuration(timeDelta)
                .build());
      }
    }
  }
